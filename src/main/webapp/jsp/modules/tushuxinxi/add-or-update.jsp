...
<body>
						...
						<div class="widget-area-2 lochana-box-shadow">
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>图书分类</label>
										<select id="tushufenleiSelect" name="tushufenlei" class="form-control">
										</select>	
									</div>	
									<div class="form-group col-md-6">
										<label>图片</label>
									...
									<div class="form-group col-md-6">
										...
										<input name="alllimittimes" id="alllimittimes" name="alllimittimes" class="form-control" placeholder="库存">
											...
											<div class="form-group  col-md-12" >
												<label>图书简介</label>
												<div><div class="upload"><input id="tushujianjieupload" name="file" type="file">选择文件</div>
	...
	<script>
		...
		var tushufenleiOptions = [];

		function tushufenleiSelect(){
			http("option/tushufenlei/tushufenlei","GET",{},(res)=>{
				if(res.code == 0){
					tushufenleiOptions = res.data;
					var nullOption = document.createElement('option');
					var tushufenleiSelect = document.getElementById('tushufenleiSelect');
					tushufenleiSelect.appendChild(nullOption);
					for(var i=0;i<tushufenleiOptions.length;i++){
						var tushufenleiOption = document.createElement('option');
						tushufenleiOption.setAttribute('name','tushufenleiOption');
						tushufenleiOption.setAttribute('value',tushufenleiOptions[i]);
						tushufenleiOption.innerHTML = tushufenleiOptions[i];									
						if(ruleForm.tushufenlei == tushufenleiOptions[i]){
							tushufenleiOption.setAttribute('selected','selected');
						}
						tushufenleiSelect.appendChild(tushufenleiOption);
					}
				}
			});
		}	
		...
		function dataBind(){
			...
			var tushufenleiOptions = document.getElementsByClassName("tushufenleiOption");
			for(var tushufenleiCount = 0; tushufenleiCount < tushufenleiOptions.length;tushufenleiCount++){
				if(tushufenleiOptions[tushufenleiCount].getAttribute('value') == ruleForm.tushufenlei){
					tushufenleiOptions[tushufenleiCount].setAttribute('selected','selected');
				}
			}
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.tushufenlei != null && res.data.tushufenlei != ''){

						var tushufenleiOptions = document.getElementsByClassName("tushufenleiOption");
						for(var tushufenleiCount = 0; tushufenleiCount < tushufenleiOptions.length;tushufenleiCount++){
							if(tushufenleiOptions[tushufenleiCount].getAttribute('value') == res.data.tushufenlei){
								tushufenleiOptions[tushufenleiCount].setAttribute('selected','selected');
							}
						}
					}
