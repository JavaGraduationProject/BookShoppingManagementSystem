...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">图书分类管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>图书分类管理</span></li>
								<li class="breadcrumb-item active"><span>图书分类列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">图书分类列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										图书分类
									</label>
									<input type="text" id="tushufenleiSearch" class="form-control form-control-sm" placeholder="请输入图书分类" aria-controls="tableId">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('tushufenlei')">图书分类<i id="tushufenleiIcon" class="fa fa-sort"></i></th>
											<th>操作</th>
										</tr>
									</thead>
									<tbody>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#tushufenleiSearch').val() != null && $('#tushufenleiSearch').val() != ''){
				searchForm.tushufenlei ="%" +  $('#tushufenleiSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("tushufenlei/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
				tushufenlei : searchForm.tushufenlei,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var tushufenleiCell  = document.createElement('td');

						 tushufenleiCell.innerHTML = item.tushufenlei;

			row.appendChild(tushufenleiCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("tushufenlei/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"tushufenlei");
